...
2. `인터럽트 방식` : MCU 자체가 하드웨어적으로 변화를 체크하여 변화 시에만 일정한 동작을 하는 방식 <br><span style="color:gray">ex) Daisy Chain, 병렬 우선순위 부여</span><br>- 제약 : 하드웨어로 원을 받아야함 <br>- 장점 : 폴링에 비해 신속한 대응 가능 => **실시간 대응** 시 유리
